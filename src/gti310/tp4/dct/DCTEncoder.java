...
public class DCTEncoder {
	...
	*/
	...
	public DCTEncoder(){
		for (int u = 0; u < _testArray.length; u++) {
			for (int v = 0; v < _testArray.length; v++) {
				double firstSomResult = 0;
				for (int i = 0; i < _testArray.length; i++) {
					double secondSomResult = 0;
					for (int j = 0; j < _testArray[i].length; j++) {
						double cosP = Math.cos((((2*i)+1)*u*Math.PI)/(_testArray.length*2));
						double cosD = Math.cos((((2*j)+1)*v*Math.PI)/(_testArray.length*2));
				...
				if(v == 0)
					...
					cv = 1;
				_encodedBloc[u][v] = (int) Math.round((((cu*cv)/(_testArray.length/2))*firstSomResult));
			}
			String output = "";
			for (int i = 0; i < _testArray.length; i++) {
					output += _encodedBloc[u][i] + ",";
			}
